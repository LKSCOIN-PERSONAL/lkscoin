            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    if (nCoinType == ONLY_DENOMINATED) { 
                        ...
                        // no coin control: send change to newly generated address = nessun controllo monete : invia la modifica all'indirizzo appena generato
                        else
                        {
                            
                            CPubKey vchPubKey; //crea una nuova chiave pubblica ?? Proviamo a inserire quella vecchia?
