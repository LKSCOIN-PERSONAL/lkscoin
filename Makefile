...
pkgdatadir = $(datadir)/lkscore
pkgincludedir = $(includedir)/lkscore
pkglibdir = $(libdir)/lkscore
pkglibexecdir = $(libexecdir)/lkscore
...
CONFIG_HEADER = $(top_builddir)/src/config/lks-config.h
CONFIG_CLEAN_FILES = liblksconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/lks-config.h.in \
...
ACLOCAL = ${SHELL} /usr/src/LKS/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /usr/src/LKS/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /usr/src/LKS/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /usr/src/LKS/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /usr/src/LKS/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = lkscore
PACKAGE_BUGREPORT = https://github.com/lkspay/lks/issues
PACKAGE_NAME = LksCoin Core
PACKAGE_STRING = LksCoin Core 0.12.2
PACKAGE_TARNAME = lkscore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /usr/src/LKS
abs_srcdir = /usr/src/LKS
abs_top_builddir = /usr/src/LKS
abs_top_srcdir = /usr/src/LKS
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /usr/src/LKS/build-aux/install-sh
...
pkgconfig_DATA = liblksconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/lksd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/lks-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/lks-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Lks-Qt.app
OSX_DMG = Lks-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_lks_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_lks_coverage.info test_lks.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Lks-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/lks-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/lks-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/lks-config.h
$(top_srcdir)/src/config/lks-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/lks-config.h src/config/stamp-h1
liblksconsensus.pc: $(top_builddir)/config.status $(srcdir)/liblksconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Lks-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Lks-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Lks-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Lks-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_lks.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_lks -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_lks_filtered.info: test_lks.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_lks_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_lks_filtered.info
...
#test_lks_coverage.info: baseline_filtered_combined.info test_lks_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_lks_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_lks_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_lks_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_lks.coverage/.dirstamp:  test_lks_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_lks.coverage/.dirstamp total.coverage/.dirstamp

# Lks: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_lks.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
